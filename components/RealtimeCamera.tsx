...
export default function RealtimeCamera() {
  ...
  useEffect(() => {
    if (running) {
      timerRef.current = setInterval(captureAndAnalyse, CAPTURE_INTERVAL_MS)
    } else {
      if (timerRef.current) clearInterval(timerRef.current)
    }
  ...
  return (
    <div className="space-y-6">
      ...
      <div className="relative rounded-2xl overflow-hidden bg-charcoal border border-border"
        ...
        <video ref={videoRef} muted playsInline
          className={`w-full h-full object-cover transition-opacity duration-500 ${running ? 'opacity-100' : 'opacity-0'}`}
        ...
        {running && (
          <motion.div initial={{ opacity: 0 }} animate={{ opacity: 1 }}
            ...
            {/* FPS */}
            <div className="flex items-center gap-1.5 bg-black/60 backdrop-blur-sm rounded-full px-3 py-1.5 border border-white/10">
              <Activity size={9} className="text-ice" />
              <span className="text-[10px] text-white font-mono">{fps} fps</span>
            </div>
            {/* Frame count */}
            <div className="bg-black/60 backdrop-blur-sm rounded-full px-3 py-1.5 border border-white/10">
              <span className="text-[10px] text-zinc-400 font-mono">{frameCount} frames</span>
            </div>
          </motion.div>
        )}
...
function FaceOverlay({ faces, videoEl }: { faces: FaceResult[]; videoEl: HTMLVideoElement | null }) {
  ...
  // The video element is mirror-flipped via CSS scaleX(-1).
  // We apply the same flip to the overlay container so boxes match.
  ...
  return (
    <div className="absolute inset-0 pointer-events-none" style={{ transform: 'scaleX(-1)' }}>
      {faces.map(face => {
        ...
        // Convert video-pixel coords to percentage
        const left   = (r.x / vW) * 100
        const top    = (r.y / vH) * 100
        const width  = (r.w / vW) * 100
        const height = (r.h / vH) * 100
